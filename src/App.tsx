...
const getDayName = (d: Date) => ['Min', 'Sen', 'Sel', 'Rab', 'Kam', 'Jum', 'Sab'][d.getDay()];
const formatLastLogin = (ts: number) => !ts ? "Belum pernah" : new Date(ts).toLocaleDateString('id-ID', { weekday: 'short', day: 'numeric', month: 'short', hour: '2-digit', minute: '2-digit' });

const SEED_ACTIVITIES = [
  { id: 'c1', name: 'Shalat Taubat', time: '02:00' }, { id: 'c2', name: 'Shalat Tahajud', time: '03:00' },
  { id: 'c3', name: 'Shalat Qabliyah Subuh', time: '04:15' }, { id: 'c4', name: 'Shalat Subuh Berjamaah', time: '04:30' },
  { id: 'c5', name: 'Dzikir Pagi', time: '05:00' }, { id: 'c6', name: 'Shalat Dhuha', time: '09:00' },
  { id: 'c7', name: 'Shalat Dzuhur Berjamaah', time: '12:00' }, { id: 'c8', name: 'Shalat Ashar Berjamaah', time: '15:15' },
  { id: 'c9', name: 'Shalat Maghrib Berjamaah', time: '18:00' }, { id: 'c10', name: 'Dzikir Petang', time: '18:30' },
  { id: 'c11', name: 'Shalat Isya Berjamaah', time: '19:15' }, { id: 'c12', name: 'Tilawah Al-Quran', time: '20:00' }
];
...
export default function IbadahTracker() {
      ...
      if (currentUser) {
         try {
            ...
            await setDoc(doc(db, 'users', currentUser.uid), {
               ...
               lastLogin: new Date().getTime(), role: isSuper ? 'superadmin' : undefined
            }, { merge: true });
         } catch (e) {}
      }
    });
  ...
  useEffect(() => {
    ...
    const unsub1 = onSnapshot(doc(db, 'users', user.uid), (snap) => {
       if (snap.exists()) {
          const d = snap.data();
          setPersonalActivities(d.activities || []); setRecords(d.records || {});
          setJournals(d.journals || []); setJoinedCommunityIds(d.joinedCommunities || []);
          if (user.email === 'coachardi1453@gmail.com') setUserRole('superadmin'); else setUserRole(d.role || 'user');
       }
       setIsSyncing(false);
    });
    const unsub2 = onSnapshot(collection(db, 'communities'), (snap) => {
       const comms: any[] = []; snap.forEach(d => comms.push({ id: d.id, ...d.data() })); setAllCommunities(comms);
    });
    const unsub3 = onSnapshot(collection(db, 'global_activities'), (snap) => {
       if (snap.empty && isSuperAdmin) { SEED_ACTIVITIES.forEach(async (act) => await setDoc(doc(db, 'global_activities', act.id), act)); } 
       else {
          const acts: any[] = []; snap.forEach(d => acts.push({ docId: d.id, ...d.data() }));
          acts.sort((a, b) => a.time.localeCompare(b.time)); setGlobalActivities(acts);
       }
    });
  ...
  useEffect(() => {
    if (isAdmin) {
      const unsub = onSnapshot(collection(db, 'users'), (snap) => {
         const usersData: any[] = []; snap.forEach(d => usersData.push({ id: d.id, ...d.data() })); setAllUsers(usersData);
      });
      return () => unsub();
    }
  }, [isAdmin]);
  ...
  const scrollToToday = () => {
      if (todayColumnRef.current && tableContainerRef.current) {
         const container = tableContainerRef.current; const target = todayColumnRef.current;
        ...
        if (comm && comm.activities) {
           comm.activities.forEach((actObj: any) => {
              ...
              if (globalAct) {
                 ...
                 if (!commActsMap[uniqueKey]) commActsMap[uniqueKey] = { id: uniqueKey, type: 'komunitas', name: globalAct.name, time: actObj.time, communities: [comm.name] };
                 else if (!commActsMap[uniqueKey].communities.includes(comm.name)) commActsMap[uniqueKey].communities.push(comm.name);
              }
           });
        }
     });
  ...
  const formattedPersonalActivities = useMemo(() => personalActivities.map(a => ({...a, type: 'pribadi'})), [personalActivities]);
  const allCombinedActivities = useMemo(() => [...formattedPersonalActivities, ...communityActivities], [formattedPersonalActivities, communityActivities]);

  const joinedCommunityNames = useMemo(() => {
     return joinedCommunityIds.map(id => allCommunities.find(c => c.id === id)?.name).filter(Boolean).join(', ');
  }, [joinedCommunityIds, allCommunities]);

  const daysInMonth = useMemo(() => {
    const year = currentDate.getFullYear(); const month = currentDate.getMonth(); const days = new Date(year, month + 1, 0).getDate();
    return Array.from({ length: days }, (_, i) => new Date(year, month, i + 1));
  }, [currentDate]);

  // --- LOGIKA TEPAT WAKTU & INPUT ---
  const getIsOnTime = (recordTimestamp: number, targetDate: Date, actTime: string) => {
     const [h] = actTime.split(':').map(Number);
     const deadline = new Date(targetDate);
     if (h < 12) deadline.setHours(12, 15, 0, 0); else { deadline.setDate(deadline.getDate() + 1); deadline.setHours(0, 30, 0, 0); }
  ...
  const handleRecord = (day: Date, actId: string, actTime: string, currentStatus: string | undefined) => {
    const now = new Date(); const [hours, minutes] = actTime.split(':').map(Number);
    ...
    if (now < unlockTime) return showToast(`Belum waktunya! Dibuka pukul ${unlockTime.toLocaleTimeString([], {hour: '2-digit', minute:'2-digit'})}`);
    ...
    let newRecs = { ...records };
    if (newStatus === 'none') delete newRecs[key]; else newRecs[key] = { status: newStatus, timestamp: now.getTime() };
    setRecords(newRecs); setHasUnsavedChanges(true); 
  };

  const saveActivity = () => {
     if (!actModal.name.trim()) return showToast("Nama aktivitas kosong!");
     let newActs = [...personalActivities];
     if (actModal.mode === 'add') newActs.push({ id: `p_${Date.now()}`, name: actModal.name, time: actModal.time }); 
     else newActs = newActs.map(a => String(a.id) === String(actModal.id) ? { ...a, name: actModal.name, time: actModal.time } : a);
     newActs.sort((a, b) => a.time.localeCompare(b.time));
     setPersonalActivities(newActs); setHasUnsavedChanges(true);
     setActModal({ show: false, mode: 'add', id: null, name: '', time: '00:00' });
     showToast(`Disimpan! Klik Simpan Perubahan.`);
  };

  const deleteActivity = (id: any) => {
     if (window.confirm("Yakin hapus aktivitas ini?")) {
        setPersonalActivities(personalActivities.filter(a => String(a.id) !== String(id))); setHasUnsavedChanges(true);
     }
  };
  ...
  const calcStats = () => {
    ...
    const calculateScore = (actArray: any[]) => {
       ...
       daysInMonth.forEach(d => {
          ...
          actArray.forEach(a => {
             ...
             if (today.getTime() >= new Date(d.getFullYear(), d.getMonth(), d.getDate(), h, m).getTime() + 60000) {
                expected++; if (records[`${dateStr}-${a.id}`]?.status === 'done') done++;
             }
          });
       });
    ...
    let totalCommScore = 0; let activeComms = 0; const communityScoresDetail: Record<string, number> = {};
    ...
    joinedCommunityIds.forEach(commId => {
       ...
       if (comm && comm.activities?.length > 0) {
          ...
          communityScoresDetail[commId] = score; totalCommScore += score; activeComms++;
       }
    });
    ...
    const actMetrics: Record<string, { name: string, type: string, done: 0, missed: 0, diffMinsTotal: 0 }> = {};
    ...
    daysInMonth.forEach(d => {
       const wIdx = Math.floor((d.getDate() - 1) / 7); const dateStr = getLocalDateStr(d);
       allCombinedActivities.forEach(a => {
          ...
          if (today.getTime() >= targetTime + 60000) {
             ...
             if (rec && rec.status === 'done') {
                weeklyStats[wIdx].done++; actMetrics[a.id].done++;
                if(isPribadi) qty.p_done++; else qty.c_done++;
                const diff = (rec.timestamp - targetTime) / 60000;
    ...
    const disciplineList = metricArray.filter(a => a.done > 0).map(a => ({ name: a.name, avgDiff: a.diffMinsTotal / a.done, done: a.done }));
    ...
    return { scorePribadi, scoreKomunitas, scoreGabungan, communityScoresDetail, qty, qtyGabungan, weeklyStats, topPribadi, botPribadi, topComm, botComm, topOnTime, topLate };
  };

  const stats = calcStats();

  const getSubDailyPct = (day: Date, acts: any[]) => {
     const today = new Date(); const dateStr = getLocalDateStr(day);
     let exp = 0; let done = 0;
     acts.forEach(a => {
        const [h, m] = a.time.split(':').map(Number);
        if (today.getTime() >= new Date(day.getFullYear(), day.getMonth(), day.getDate(), h, m).getTime() + 60000) {
           exp++; if (records[`${dateStr}-${a.id}`]?.status === 'done') done++;
        }
     });
  ...
  const saveToServer = async (isAutoSave = false) => {
    ...
    const payload: any = { 
       activities: personalActivities, records, journals, joinedCommunities: joinedCommunityIds,
       displayName: user.displayName, email: user.email, lastActivity: new Date().getTime(),
       [`score_${mKey}_gabungan`]: stats.scoreGabungan
    };
    Object.entries(stats.communityScoresDetail).forEach(([cId, score]) => payload[`score_${mKey}_comm_${cId}`] = score);
    try {
      await setDoc(doc(db, 'users', user.uid), payload, { merge: true });
      setHasUnsavedChanges(false); if(!isAutoSave) showToast("Disinkronkan ke Server!");
    } catch (e) { showToast("Tersimpan lokal. Akan sync saat online."); } 
  ...
  const executeRoleChange = async () => {
     try {
        await setDoc(doc(db, 'users', roleConfirmModal.targetUser.id), { role: roleConfirmModal.makeAdmin ? 'admin' : 'user' }, { merge: true });
  ...
  const handleSaveCommunity = async () => {
     if (!newCommName.trim() || selectedActs.length === 0) return showToast("Nama & min 1 aktivitas wajib!");
     try {
        if (editCommId) {
           await updateDoc(doc(db, 'communities', editCommId), { name: newCommName, activities: selectedActs });
           ...
           await addDoc(collection(db, 'communities'), { name: newCommName, ownerId: user.uid, activities: selectedActs, joinCode, createdAt: new Date().getTime() });
           showToast(`Komunitas dibuat! Kode: ${joinCode}`);
        }
  ...
  const handleDeleteCommunity = async (id: string) => {
     if(window.confirm("PERINGATAN: Yakin menghapus komunitas ini selamanya?")) {
        try { await deleteDoc(doc(db, 'communities', id)); showToast("Komunitas dihapus."); } 
  ...
  const handleAddGlobalActivity = async () => {
     ...
     try {
        ...
        await setDoc(doc(db, 'global_activities', newId), { id: newId, name: newGlobalAct.name, time: newGlobalAct.time });
        setNewGlobalAct({ name: '', time: '00:00' }); showToast("Master ditambahkan.");
  ...
  const handleJoinCommunity = () => {
     ...
     setJoinedCommunityIds([...joinedCommunityIds, comm.id]); setHasUnsavedChanges(true); setShowJoinModal(false); setJoinCodeInput('');
  ...
  useEffect(() => {
     if(allCommunities.length > 0 && joinedCommunityIds.length > 0) {
        ...
        if(validIds.length !== joinedCommunityIds.length) setJoinedCommunityIds(validIds);
     }
  ...
  const filteredAdminUsers = useMemo(() => {
     ...
     if (adminSearch) result = result.filter(u => (u.displayName||'').toLowerCase().includes(adminSearch.toLowerCase()) || (u.email||'').toLowerCase().includes(adminSearch.toLowerCase()));
  ...
  const linearRegression = (data: any[], key: string) => {
     ...
     data.forEach(d => { sumX += d.x; sumY += d[key]; sumXY += d.x * d[key]; sumX2 += d.x * d.x; });
     const den = (n * sumX2 - sumX * sumX); const m = den === 0 ? 0 : (n * sumXY - sumX * sumY) / den;
     const b = (sumY - m * sumX) / n; return {m, b};
  };
  ...
  const mainChartData = useMemo(() => {
    const raw = daysInMonth.map((d, i) => {
      ...
      return { x: i + 1, tgl: d.getDate().toString(), pri: p==='-'?0:parseInt(p), kom: c==='-'?0:parseInt(c), gab: g==='-'?0:parseInt(g) };
    });
    const tPri = linearRegression(raw, 'pri'); const tKom = linearRegression(raw, 'kom'); const tGab = linearRegression(raw, 'gab');
    return raw.map(d => ({ ...d, t_pri: Math.max(0,Math.min(100, Math.round(tPri.m*d.x+tPri.b))), t_kom: Math.max(0,Math.min(100, Math.round(tKom.m*d.x+tKom.b))), t_gab: Math.max(0,Math.min(100, Math.round(tGab.m*d.x+tGab.b))) }));
  }, [daysInMonth, records, formattedPersonalActivities, communityActivities, allCombinedActivities]);

  const wtwData = useMemo(() => {
     const targetDate = new Date(); targetDate.setMonth(targetDate.getMonth() + wtwMonthOffset);
     const days = new Date(targetDate.getFullYear(), targetDate.getMonth() + 1, 0).getDate();
     const wStats = [ { name: 'Pekan 1', exp:0, done:0 }, { name: 'Pekan 2', exp:0, done:0 }, { name: 'Pekan 3', exp:0, done:0 }, { name: 'Pekan 4', exp:0, done:0 }, { name: 'Pekan 5', exp:0, done:0 } ];
     
     Array.from({ length: days }, (_, i) => new Date(targetDate.getFullYear(), targetDate.getMonth(), i + 1)).forEach(d => {
        const wIdx = Math.floor((d.getDate() - 1) / 7); const dStr = getLocalDateStr(d);
        allCombinedActivities.forEach(a => {
           const [h,m] = a.time.split(':').map(Number);
           if (new Date().getTime() >= new Date(d.getFullYear(), d.getMonth(), d.getDate(), h, m).getTime() + 60000) {
              wStats[wIdx].exp++; if(records[`${dStr}-${a.id}`]?.status === 'done') wStats[wIdx].done++;
           }
        });
     });
     return wStats.filter(w => w.exp > 0).map(w => ({ name: w.name, pencapaian: Math.round((w.done/w.exp)*100) }));
  ...
  const mtmData = useMemo(() => {
     ...
     for(let i = mtmRange - 1; i >= 0; i--) {
        ...
        // Rekonstruksi skor bulan tersebut jika ada di profil
        const userProfileScore = user && allUsers.find(u => u.id === user.uid)?.[`score_${mKey}_gabungan`];
        result.push({ x: mtmRange-i, bln: MONTH_NAMES[d.getMonth()].substring(0,3), skor: userProfileScore || 0 });
     }
     const tLine = linearRegression(result, 'skor');
     return result.map(d => ({ ...d, trend: Math.max(0,Math.min(100, Math.round(tLine.m*d.x+tLine.b))) }));
    ...
    <div className="fixed inset-0 w-full h-full overflow-y-auto bg-slate-50 text-slate-800 font-sans">
      <div className="min-h-full p-2 md:p-6 relative">
           ...
           <div className="fixed inset-0 bg-slate-900/80 backdrop-blur-sm flex items-center justify-center z-[120] p-4">
              <div className="bg-white rounded-3xl p-6 md:p-8 w-full max-w-lg shadow-2xl relative max-h-[90vh] overflow-y-auto">
                 ...
                 <div className="flex items-center gap-3 mb-6">
                    ...
                    <div><h2 className="text-xl font-bold text-slate-800">Tracker IbadahKU</h2><p className="text-xs text-orange-600 font-bold tracking-widest">VER 20.08.26 rev4</p></div>
                 </div>
        ...
        {roleConfirmModal.show && (
           <div className="fixed inset-0 bg-slate-900/80 backdrop-blur-sm flex items-center justify-center z-[130] p-4">
              <div className="bg-white rounded-3xl p-6 w-full max-w-sm shadow-2xl relative text-center">
                 <div className="w-16 h-16 bg-orange-100 rounded-full flex items-center justify-center mx-auto mb-4"><Shield className="text-orange-600" size={32}/></div>
                 ...
                 <div className="flex gap-3">
                    ...
                    <button onClick={executeRoleChange} className="flex-1 bg-orange-600 text-white font-bold py-3 rounded-xl hover:bg-orange-700">Ya, Ubah Status</button>
                 </div>
              </div>
           </div>
        )}
        ...
        {showJoinModal && (
           <div className="fixed inset-0 bg-slate-900/80 backdrop-blur-sm flex items-center justify-center z-[110] p-4">
              <div className="bg-white rounded-3xl p-6 w-full max-w-md shadow-2xl relative text-center">
                 ...
                 <div className="w-16 h-16 bg-purple-100 rounded-full flex items-center justify-center mx-auto mb-4"><KeyRound size={32} className="text-purple-600"/></div>
           ...
           <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm flex items-center justify-center z-[60] p-4">
              <div className="bg-white rounded-3xl p-6 w-full max-w-md shadow-2xl relative">
                 ...
                 <div className="space-y-4 mb-6">
                    <div><label className="block text-xs font-bold text-slate-600 mb-1">Nama</label><input type="text" value={actModal.name} onChange={e => setActModal({...actModal, name: e.target.value})} className="w-full bg-slate-50 border border-slate-200 rounded-xl p-3 focus:border-blue-500 outline-none" /></div>
                    <div><label className="block text-xs font-bold text-slate-600 mb-1">Jam</label><input type="time" value={actModal.time} onChange={e => setActModal({...actModal, time: e.target.value})} className="w-full bg-slate-50 border border-slate-200 rounded-xl p-3 focus:border-blue-500 outline-none" /></div>
                 </div>
                       ...
                       <div className="overflow-x-auto border border-slate-100 rounded-xl">
                          <table className="w-full text-left text-sm min-w-[800px]">
                             <thead className="bg-slate-50 text-slate-600 border-b border-slate-100">
                                <tr><th className="p-4 font-bold">Nama</th><th className="p-4 font-bold">Email</th><th className="p-4 font-bold">Status (Aktif/Login)</th>{isSuperAdmin && <th className="p-4 font-bold text-center">Hak Akses</th>}</tr>
                             </thead>
                             <tbody className="divide-y divide-slate-50">
                                {filteredAdminUsers.map((u) => {
                                   ...
                                   if (u.lastActivity || u.lastLogin) {
                                      ...
                                      if (diffDays > 2) actStatus = <span className="text-red-600 font-medium text-[11px]"><span className="font-bold">⚠️ Pasif {diffDays} Hari</span><br/><span className="text-[9px]">{formatLastLogin(timestamp)}</span></span>;
                                      else actStatus = <span className="text-green-600 font-medium text-[11px]"><span className="font-bold">🟢 Aktif</span><br/><span className="text-[9px]">{formatLastLogin(timestamp)}</span></span>;
                                   }
                                   return (
                                   <tr key={u.id} className="hover:bg-slate-50/50">
                                      <td className="p-4 font-bold text-slate-800">{u.displayName || 'Anonim'}</td><td className="p-4 text-slate-500 text-xs">{u.email || '-'}</td><td className="p-4">{actStatus}</td>
                                      {isSuperAdmin && (
                                         <td className="p-4 text-center">
                                            <button onClick={() => setRoleConfirmModal({show:true, targetUser:u, makeAdmin: u.role!=='admin'})} disabled={u.email === 'coachardi1453@gmail.com'} className={`px-3 py-1 rounded text-[10px] font-bold uppercase transition-colors ${u.role === 'admin' ? 'bg-orange-100 text-orange-700 border border-orange-200' : 'bg-slate-100 text-slate-500 hover:bg-slate-200'}`}>{u.role === 'admin' ? 'Admin' : 'User'}</button>
                                         </td>
                                      )}
                                   </tr>
                                )})}
                          ...
                          <div className="space-y-3">
                             {allCommunities.filter(c => c.ownerId === user.uid).map(c => (
                                <div key={c.id} className="bg-white border-2 border-dashed border-slate-200 p-4 rounded-xl flex items-center justify-between group">
                                   <div>
                                      <p className="font-bold text-slate-800 text-lg flex items-center gap-2">{c.name} 
        ...
        <div className="max-w-7xl mx-auto space-y-8 pb-16">
          ...
          {/* HEADER */}
          <div className="bg-white rounded-2xl shadow-sm border border-slate-200 p-4 sm:p-6 flex justify-between items-center gap-4 relative overflow-hidden">
            <div className="flex items-center gap-3 sm:gap-4">
              <div className="w-12 h-12 sm:w-16 sm:h-16 bg-white rounded-full border-2 border-orange-500 flex items-center justify-center shadow-md"><img src="/logo.png" alt="Logo" className="w-[70%] h-[70%] object-contain" /></div>
              <div><h1 className="text-lg sm:text-2xl font-bold text-slate-900 tracking-wide">Tafkir Corp</h1><p className="text-[9px] sm:text-xs text-orange-600 font-bold uppercase tracking-[0.2em] mt-1">Tracker Ibadah & Hal Positif</p></div>
            </div>
            <div className="flex items-center gap-2 sm:gap-4">
               <div className="text-right flex flex-col justify-center bg-slate-50 px-3 py-1.5 rounded-xl border border-slate-100 hidden sm:flex">
                 ...
                 <div className="text-[10px] font-bold flex justify-end">{isSyncing ? <span className="text-blue-500 flex items-center gap-1"><RefreshCw size={10} className="animate-spin"/> Syncing...</span> : <span className="text-green-600 flex items-center gap-1"><Zap size={10}/> Synchronized</span>}</div>
               </div>
               ...
               <button onClick={()=>signOut(auth)} className="bg-red-50 text-red-600 hover:bg-red-100 p-2 sm:px-3 sm:py-1.5 rounded-lg transition-colors text-sm font-medium flex items-center gap-1 border border-red-100"><LogOut size={16}/> <span className="hidden sm:block">Keluar</span></button>
            </div>
          </div>
               ...
               <div className="flex flex-col md:flex-row justify-between w-full gap-4 items-center">
                  <div className="flex flex-col items-start gap-2 w-full md:w-auto">
                     <div className="flex items-center gap-3">
                        <h2 className="text-lg font-bold text-slate-800 flex items-center gap-2"><span className="w-3 h-3 rounded-full bg-orange-500"></span> Tabel Hisab </h2>
                        <button onClick={() => setActModal({ show: true, mode: 'add', id: null, name: '', time: '00:00' })} className="bg-blue-50 border border-blue-200 px-2 py-1 rounded text-[10px] font-bold text-blue-600 hover:bg-blue-100 flex items-center gap-1">+ Buat Sendiri</button>
                     ...
                     {joinedCommunityNames && <p className="text-[10px] font-semibold text-slate-500 uppercase flex items-center gap-1"><Users size={12}/> Komunitas Anda: {joinedCommunityNames}</p>}
                  </div>
                  ...
                  <div className="flex items-center gap-2 sm:gap-4 bg-white border border-slate-200 px-2 py-1 rounded-lg shadow-sm">
                     ...
                     <div className="w-32 sm:w-40 text-center font-bold text-slate-700 text-sm sm:text-base">{MONTH_NAMES[currentDate.getMonth()]} {currentDate.getFullYear()}</div>
            ...
            <div className="overflow-x-auto pb-4" ref={tableContainerRef}>
              <table className="w-full text-sm">
                <thead className="bg-slate-100 border-b border-slate-200">
                  <tr>
                    <th className="text-left text-slate-700 font-bold p-3 sticky left-0 bg-slate-100 z-20 shadow-[2px_0_5px_-2px_rgba(0,0,0,0.1)] min-w-[180px] sm:min-w-[280px]">Ibadah & Aktivitas Positif KU</th>
                    {daysInMonth.map(d => {
                ...
                <tbody>
                  <tr>
                     <td colSpan={daysInMonth.length + 1} className="bg-blue-50 border-y border-blue-100 p-2 sticky left-0 z-10"><span className="font-bold text-blue-800 text-xs uppercase tracking-widest pl-2">Komitmen Pribadi</span></td>
                  </tr>
                    ...
                    <tr key={act.id} className="bg-white hover:bg-orange-50/50 border-b border-slate-50">
                      <td className="p-2 font-medium sticky left-0 z-10 shadow-[2px_0_5px_-2px_rgba(0,0,0,0.05)] bg-white">
                        <div className="flex items-center gap-2">
                           <div className="flex flex-col gap-1"><button onClick={() => setActModal({ show:true, mode:'edit', id:act.id, name:act.name, time:act.time })} className="text-slate-400 hover:text-blue-600"><Edit3 size={14}/></button><button onClick={() => deleteActivity(act.id)} className="text-slate-400 hover:text-red-600"><Trash2 size={14}/></button></div>
                           <div className="flex flex-col items-start min-w-0">
                              <div className="flex items-center gap-1"><span className={`text-xs font-bold truncate ${!isSafe ? 'text-red-600' : 'text-slate-700'}`}>{act.name}</span>
                                {!isSafe && <div className="relative group/alert cursor-help"><AlertTriangle size={12} className="text-red-500" /><div className="absolute hidden group-hover/alert:block bottom-full left-1/2 -translate-x-1/2 mb-2 w-40 bg-slate-900 text-white text-[9px] text-center p-2 rounded shadow-xl z-50 pointer-events-none">Pelaksanaan di bawah 50%</div></div>}
                              </div>
                      ...
                      {daysInMonth.map(d => {
                        ...
                        return (
                          <td key={`${d}-${act.id}`} className="p-1 text-center relative group cursor-pointer border-r border-slate-50" onClick={() => handleRecord(d, act.id, act.time, rec?.status)}>
                            ...
                            {rec && <div className="absolute hidden group-hover:block bottom-full left-1/2 -translate-x-1/2 mb-2 bg-slate-900 text-white text-[10px] p-2 rounded shadow-xl w-max z-30 pointer-events-none">
                                <p className="font-semibold">{act.name}</p><p className="text-slate-300">Isi: {new Date(rec.timestamp).toLocaleTimeString('id-ID', {hour:'2-digit', minute:'2-digit'})}</p>
                  ...
                  <tr className="bg-blue-50/30 border-t border-blue-100">
                    ...
                    {daysInMonth.map(d => <td key={d.toISOString()} className="p-2 text-center font-bold text-blue-700 text-[10px]">{getSubDailyPct(d, formattedPersonalActivities)}</td>)}
                  </tr>
                </tbody>
                ...
                <tbody>
                  <tr>
                     <td colSpan={daysInMonth.length + 1} className="bg-purple-50 border-y border-purple-100 p-2 sticky left-0 z-10"><span className="font-bold text-purple-800 text-xs uppercase tracking-widest pl-2">Komitmen Komunitas</span></td>
                  </tr>
                      ...
                      <td className="p-2 font-medium sticky left-0 z-10 shadow-[2px_0_5px_-2px_rgba(0,0,0,0.05)] bg-[#f8fafc]">
                        <div className="flex items-center gap-2 pl-2">
                           <div className="flex flex-col min-w-0">
                              <div className="flex items-center gap-1"><span className={`text-xs font-bold truncate ${!isSafe ? 'text-red-600' : 'text-slate-800'}`}>{act.name}</span>
                                {!isSafe && <div className="relative group/alert cursor-help"><AlertTriangle size={12} className="text-red-500" /><div className="absolute hidden group-hover/alert:block bottom-full left-1/2 -translate-x-1/2 mb-2 w-40 bg-slate-900 text-white text-[9px] text-center p-2 rounded shadow-xl z-50 pointer-events-none">Pelaksanaan di bawah 50%</div></div>}
                              </div>
                  ...
                  <tr className="bg-purple-50/30 border-t border-purple-100">
                    ...
                    {daysInMonth.map(d => <td key={d.toISOString()} className="p-2 text-center font-bold text-purple-700 text-[10px]">{getSubDailyPct(d, communityActivities)}</td>)}
                  </tr>
                </tbody>
                ...
                <tfoot>
                  <tr className="bg-slate-200 border-t-2 border-slate-300">
                    ...
                    {daysInMonth.map(d => <td key={d.toISOString()} className="p-2 text-center font-black text-slate-800 border-r border-slate-300 text-xs sm:text-sm">{getDailyPercentage(d)}</td>)}
                  </tr>
                </tfoot>
              </table>
            </div>
          ...
          {joinedCommunityIds.length > 0 && (
             <div className="space-y-6">
                ...
                {joinedCommunityIds.map(commId => {
                   const comm = allCommunities.find(c => c.id === commId); if (!comm) return null;
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                               {boardData.map((usr, idx) => (
                                  <div key={idx} className={`flex items-center justify-between p-3 rounded-xl border ${idx === 0 ? 'bg-gradient-to-r from-yellow-500/20 border-yellow-500/30' : idx === 1 ? 'bg-gradient-to-r from-slate-300/10 border-slate-400/20' : idx === 2 ? 'bg-gradient-to-r from-orange-600/20 border-orange-500/20' : 'bg-white/5 border-white/10'}`}>
                                     <div className="flex items-center gap-3"><div className={`w-8 h-8 rounded-full flex items-center justify-center font-black text-sm shadow-md ${idx === 0 ? 'bg-yellow-400 text-yellow-900' : idx === 1 ? 'bg-slate-300 text-slate-800' : idx === 2 ? 'bg-orange-400 text-orange-900' : 'bg-slate-800 text-slate-400'}`}>{idx + 1}</div><span className={`font-bold text-sm truncate max-w-[150px] ${idx === 0 ? 'text-yellow-400' : 'text-slate-200'}`}>{usr.name}</span></div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            <div className="bg-white border border-slate-200 rounded-2xl shadow-sm p-6 flex flex-col">
              <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 mb-4 border-b border-slate-100 pb-4">
                 ...
                 <div className="flex items-center gap-2 w-full sm:w-auto">
                    <div className="relative w-full sm:w-32"><Search size={14} className="absolute left-2 top-1/2 -translate-y-1/2 text-slate-400" /><input type="text" placeholder="Cari..." value={journalSearch} onChange={e => setJournalSearch(e.target.value)} className="w-full bg-slate-50 border border-slate-200 rounded-lg pl-7 pr-2 py-1.5 text-xs outline-none" /></div>
                    <select value={journalSort} onChange={(e: any) => setJournalSort(e.target.value)} className="bg-slate-50 border border-slate-200 rounded-lg px-2 py-1.5 text-xs outline-none"><option value="newest">Terbaru</option><option value="oldest">Terlama</option><option value="az">A - Z</option><option value="za">Z - A</option></select>
                 </div>
              </div>
              <div className="space-y-3 flex-1 overflow-y-auto max-h-[300px] pr-2">
                {filteredAndSortedJournals.length === 0 ? <p className="text-sm text-slate-400 text-center py-8">Kosong.</p> : filteredAndSortedJournals.map(j => (
                  <div key={j.id} className="bg-slate-50 p-3 rounded-xl border flex justify-between items-start group hover:border-orange-300 transition-colors">
                    <div className="truncate pr-2"><p className="font-bold text-slate-700 truncate">{j.title}</p><p className="text-xs text-slate-500 mt-1">{new Date(j.date).toLocaleDateString('id-ID')}</p></div>
                    <div className="flex gap-2 opacity-50 group-hover:opacity-100"><button onClick={() => { setActiveJournal(j); setIsViewModalOpen(true); }} className="text-blue-600"><Eye size={16} /></button><button onClick={() => { setActiveJournal(j); setJournalInput({title: j.title, content: j.content}); setIsViewModalOpen(false); }} className="text-orange-600"><Edit3 size={16} /></button><button onClick={() => { setJournals(journals.filter(x => x.id !== j.id)); setHasUnsavedChanges(true); }} className="text-red-600"><Trash2 size={16} /></button></div>
                  </div>
                ))}
              </div>
            </div>
            <div className="lg:col-span-2 bg-white border border-slate-200 rounded-2xl shadow-sm p-6">
              ...
              <div className="flex justify-end"><button onClick={saveJournal} className="bg-slate-800 text-white px-6 py-2.5 rounded-xl hover:bg-slate-900 font-bold shadow-md flex items-center gap-2"><Check size={18}/> Draf Jurnal</button></div>
            </div>
          </div>
          ...
          <div ref={chartRef} className="bg-white border border-slate-200 rounded-2xl shadow-sm p-6 sm:p-8 relative overflow-hidden">
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-8 gap-4">
               <div><h2 className="text-xl font-bold text-slate-800 border-l-4 border-orange-500 pl-4">Dashboard Analisa Eksekutif</h2><p className="text-sm text-slate-500 mt-2 pl-4 font-medium">Laporan: <span className="font-bold text-slate-700">{user.displayName}</span></p></div>
                     ...
                     <div className="bg-white p-3 rounded-xl border border-blue-100 shadow-sm">
                        ...
                        <div className="space-y-1">
                           {stats.topOnTime.length ? stats.topOnTime.map((a,i) => (
                              <div key={i} className="flex justify-between text-[10px]"><span className="truncate font-semibold text-slate-700">{i+1}. {a.name}</span><span className="text-blue-600 font-bold">{Math.round(a.avgDiff)}m</span></div>
                     ...
                     <div className="bg-white p-3 rounded-xl border border-orange-100 shadow-sm">
                        ...
                        <div className="space-y-1">
                           {stats.topLate.length ? stats.topLate.map((a,i) => (
                              <div key={i} className="flex justify-between text-[10px]"><span className="truncate font-semibold text-slate-700">{i+1}. {a.name}</span><span className="text-orange-500 font-bold">{Math.round(a.avgDiff)}m</span></div>
                           )) : <p className="text-xs text-center italic text-slate-400">Belum ada</p>}
                        </div>
                     </div>
                  </div>
               ...
               <div className="bg-slate-50 rounded-2xl p-6 border border-slate-200">
                  <div className="flex justify-between items-center mb-4">
                     ...
                     <select value={wtwMonthOffset} onChange={e=>setWtwMonthOffset(Number(e.target.value))} className="text-xs bg-white border border-slate-200 rounded p-1">
                        <option value={0}>Bulan Ini</option><option value={-1}>Bulan Lalu</option><option value={-2}>2 Bln Lalu</option>
                     </select>
                  </div>
                  <div className="h-48">
                     <ResponsiveContainer width="100%" height="100%">
               ...
               <div className="bg-slate-50 rounded-2xl p-6 border border-slate-200">
                  <div className="flex justify-between items-center mb-4">
                     ...
                     <select value={mtmRange} onChange={e=>setMtmRange(Number(e.target.value))} className="text-xs bg-white border border-slate-200 rounded p-1">
                        <option value={3}>3 Bln Terakhir</option><option value={6}>6 Bln Terakhir</option><option value={12}>1 Tahun</option>
                     </select>
                  </div>
                  <div className="h-48">
                     <ResponsiveContainer width="100%" height="100%">
